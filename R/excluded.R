...
eipps_type <- function(x) {
  Type(
    code = x,
    abbr = eipps_type_abbr(x),
    desc = eipps_type_desc(x)
  )
}
...
eipps_parent <- function(x) {
  EippsParent(code = substr_(x, 3L), ccn = eipps_parent_ccn(x))
}
...
EippsUnit2 <- S7::new_class(
  name = "EippsUnit2",
  parent = EippsUnit,
  properties = list(parent = EippsParent)
)

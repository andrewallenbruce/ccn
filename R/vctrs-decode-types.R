...
decode_medicaid_type <- function(x) {
  vctrs::vec_recode_values(
    ...
    to = vctrs::vec_c(
      vctrs::vec_rep("SNF-NF", 4L),
      vctrs::vec_rep("ICF-IID", 2L),
      "MOH",
      "HHA",
      "PRTF",
      NA_character_
...
unit_type_infix <- function(x) {
  vctrs::vec_recode_values(
    x,
    from = vctrs::vec_c("M", "R", "U", "W", "Y", "Z", NA_character_),
    to = vctrs::vec_c("1", "1", "0", "2", "3", "1", NA_character_)
  )
}
...
decode_supplier_type <- function(x) {
  vctrs::vec_recode_values(
    x,
    from = vctrs::vec_c("C", "D", "X", NA_character_),
    to = vctrs::vec_c("ASC", "CLIA", "PXRF", NA_character_)
  )
}
...
decode_emergency_type <- function(x) {
  vctrs::vec_recode_values(
    x,
    from = vctrs::vec_c("E", "F", NA_character_),
    to = vctrs::vec_c("NON", "FED", NA_character_)
  )
}
...
decode_organ_type <- function(x) {
  vctrs::vec_recode_values(
    x,
    from = vctrs::vec_c("P", NA_character_),
    to = vctrs::vec_c("OPO", NA_character_)
  )
}
